...
struct BTreeNode {
    ...
    void serialize(uint8_t* buffer) const {
        ...
        memcpy(buffer + offset, &num_keys, sizeof(num_keys)); offset += sizeof(num_keys);
        memcpy(buffer + offset, &parent_page, sizeof(parent_page)); offset += sizeof(parent_page);
        memcpy(buffer + offset, &next_leaf, sizeof(next_leaf)); offset += sizeof(next_leaf);
        
        // Write keys
        memcpy(buffer + offset, keys, sizeof(keys)); offset += sizeof(keys);
    ...
    void deserialize(const uint8_t* buffer) {
        ...
        memcpy(&num_keys, buffer + offset, sizeof(num_keys)); offset += sizeof(num_keys);
        memcpy(&parent_page, buffer + offset, sizeof(parent_page)); offset += sizeof(parent_page);
        memcpy(&next_leaf, buffer + offset, sizeof(next_leaf)); offset += sizeof(next_leaf);
        
        // Read keys
        memcpy(keys, buffer + offset, sizeof(keys)); offset += sizeof(keys);
