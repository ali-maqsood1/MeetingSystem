...
export default function ChatPanel({ meetingId }) {
  ...
  const loadMessages = async () => {
    try {
      ...
      if (data.success) {
        const parsed = typeof data.messages === 'string'
  ...
  const sendMessage = async () => {
    ...
    try {
      const data = await api.sendMessage(meetingId, inputMessage);
      if (data.success) {
        setInputMessage('');
        await loadMessages();
      }
  ...
  return (
    <div className="h-full flex flex-col bg-dark-900">
      ...
      <div className="px-6 py-4 border-b border-dark-700 bg-dark-800/50 backdrop-blur-sm">
        ...
        <p className="text-sm text-gray-400 mt-1">{messages.length} message(s)</p>
      </div>
              ...
              >
                <div className={`max-w-md ${isMe ? 'order-2' : 'order-1'}`}>
                  <div className="flex items-center gap-2 mb-1">
                    <span className={`text-xs font-semibold ${isMe ? 'text-green-400' : 'text-primary-400'}`}>
