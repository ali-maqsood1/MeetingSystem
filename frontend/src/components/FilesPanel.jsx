...
import { Upload, Download, File, Trash2, FileText, Image, Video, Music, AlertCircle, Loader } from 'lucide-react';
...
export default function FilesPanel({ meetingId }) {
  ...
  const loadFiles = async () => {
    try {
      ...
      if (data.success) {
        const filesArray = typeof data.files === 'string' 
    ...
    try {
      ...
      reader.onload = async (e) => {
        try {
          ...
          const data = await api.uploadFile(meetingId, selectedFile.name, base64Data);
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-4 mt-2 text-sm text-gray-400">
                      ...
                      <span>•</span>
                      <span>{new Date(file.uploaded_at * 1000).toLocaleString()}</span>
                    </div>
                  </div>
